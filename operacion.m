function resultado = operacion(vector)
    ...
    elseif(length(ordenar)< 4)%se revisa que el largo del vector no se inferior a 4
        ...
        menores = sum(sqrt(ordenar(1:4))); %se obtienen los 4 menores numeros con su raiz respectiva y se suman
        mayores = sqrt(sum(ordenar(end-3:end))); %se obtienen los 4 mayores numeros se suman y obtenemos su raiz cuadrada
        resultado = mayores - menores;% se obtiene el resultado a la operación
    end
